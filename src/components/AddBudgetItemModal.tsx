...
const matchesSubcategory = (existingId: string, subId: string) =>
	existingId === subId || existingId === subId.replace(/^sub_/, "") || `sub_${existingId}` === subId;

export default function AddBudgetItemModal({ isOpen, onClose, onAdd, sectionName, existingSubcategoryIds }: AddBudgetItemModalProps) {
	...
	const availableSubcategories = subcategories.filter((s) => {
		const matchesSection = s.categoryName != null && s.categoryName.toLowerCase().trim() === sectionName.toLowerCase().trim();
		const notAlreadyInSection = !existingSubcategoryIds.some((eid) => matchesSubcategory(eid, s.id));
	...
	return (
		<div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto p-4">
			...
			<div className="relative z-10 w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
				<h2 className="mb-6 text-xl font-bold text-gray-800">Add Budget Item</h2>
				<p className="mb-4 text-sm text-gray-600">Add a new item to {sectionName}</p>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						<div>
							...
							{availableSubcategories.length === 0 && !loading && (
								<p className="mt-1 text-sm text-amber-600">No subcategories available for this section.</p>
							)}
						</div>

						<div className="grid grid-cols-2 gap-4">
							<div>
								<label htmlFor="monthly" className="mb-1.5 block text-sm font-medium text-gray-700">
									Monthly
								</label>
								<input
									id="monthly"
							...
							<div>
								<label htmlFor="annual" className="mb-1.5 block text-sm font-medium text-gray-700">
									Annual
								</label>
								<input
									id="annual"
