...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (listOwners) {
			const { data, error } = await supabase.from("budgets").select("owner").order("owner");
		...
		let query = supabase.from("budgets").select("year").order("year", { ascending: false });
		...
		return NextResponse.json({ error: "Failed to fetch budget years" }, { status: 500 });
	}
}
...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const { data: existing } = await supabase.from("budgets").select("id").eq("id", id).maybeSingle();

		if (existing) {
			return NextResponse.json({ error: `Budget already exists for ${owner} - ${yearNum}` }, { status: 409 });
		}
		...
		if (copyFromOwner && copyFromYear) {
			...
			const { data: copyBudget } = await supabase.from("budgets").select("annual_income, other_income, estimated_taxes").eq("id", copyId).single();
		...
		if (copyFromOwner && copyFromYear) {
			...
			const { data: copyAllocs } = await supabase
				...
				.select("subcategory_id, monthly_budget, annual_budget")
			...
			if (copyAllocs && copyAllocs.length > 0) {
				...
				const { error: allocError } = await supabase.from("budget_allocations").insert(newAllocs);
