...
export default function DashboardPage() {
	...
	const fetchRecentMonthBreakdown = useCallback(async () => {
		...
		try {
			const res = await fetch(`/api/dashboard/subcategory-breakdown?year=${yearNum}&month=${month}`);
	...
	return (
		<div className="mx-auto max-w-7xl px-6 py-8">
			...
			<div className="mb-10">
				...
				<div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
					{recentMonthLoading ? (
						...
						<table className="w-full">
							<thead>
								<tr className="bg-gray-50">
									...
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Monthly Budget</th>
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Budget Left</th>
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Budget Usage</th>
								</tr>
							</thead>
							<tbody>
								{recentMonthBreakdown.rows.map((row, i) => (
									<tr key={i} className="border-t border-gray-100 hover:bg-gray-50">
										<td className={`px-4 py-3 text-sm ${subCategoryNameTextClass(row.subCategory)}`}>{row.subCategory}</td>
										{recentMonthBreakdown.owners.map((owner) => (
											<td key={owner} className="px-4 py-3 text-sm text-gray-700">
												{formatCurrency(row.byOwner[owner] ?? 0)}
											</td>
										))}
										<td className="px-4 py-3 text-sm text-gray-700">{formatCurrency(row.total)}</td>
										<td className="px-4 py-3 text-sm text-gray-700">{formatCurrency(row.budget)}</td>
										<td className="px-4 py-3 text-sm text-gray-700">{formatCurrency(row.budgetLeft)}</td>
										<td className="px-4 py-3">
											<div className="flex items-center gap-2">
												...
												<span className="text-xs text-gray-500">{row.budgetUsed}% used</span>
											</div>
										</td>
									</tr>
								))}
								<tr className="border-t border-gray-200 bg-green-50 font-semibold">
									...
									<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(recentMonthBreakdown.totals.total)}</td>
									<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(recentMonthBreakdown.totals.budget)}</td>
									<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(recentMonthBreakdown.totals.budgetLeft)}</td>
									<td className="px-4 py-3">
										<div className="flex items-center gap-2">
											<div className="h-2 flex-1 max-w-[100px] overflow-hidden rounded-full bg-gray-200">
												<div
											...
											<span className="text-xs text-gray-500">{recentMonthBreakdown.totals.budgetUsed}% used</span>
										</div>
									</td>
								</tr>
							</tbody>
						...
						<div className="py-12 text-center text-gray-500">No transactions for this month</div>
					)}
				</div>
			</div>
			...
			<div className="mb-10">
				<h2 className="mb-6 text-xl font-bold text-gray-800">Year to Date Spending</h2>
				<div className="mb-8 grid gap-6 lg:grid-cols-2">
					<div className="rounded-xl bg-white p-6 shadow-sm">
						<h3 className="text-lg font-semibold text-gray-800">Spending vs Budget</h3>
					...
					<div className="rounded-xl bg-white p-6 shadow-sm">
						...
						<p className="mb-4 text-sm text-gray-500">This month&apos;s spending by category</p>
				...
				<div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
					{monthlyBreakdownsLoading ? (
						...
						<table className="w-full">
							<thead>
								<tr className="bg-gray-50">
									...
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Total</th>
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">YTD Budget</th>
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Budget Left</th>
									<th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Budget Usage</th>
								</tr>
							</thead>
							<tbody>
								...
								<tr className="border-t border-gray-200 bg-green-50 font-semibold">
									...
									<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(monthlyBreakdowns.ytd.totals.total)}</td>
									<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(monthlyBreakdowns.ytd.totals.budget)}</td>
									<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(monthlyBreakdowns.ytd.totals.budgetLeft)}</td>
									<td className="px-4 py-3">
										<div className="flex items-center gap-2">
											<div className="h-2 flex-1 max-w-[100px] overflow-hidden rounded-full bg-gray-200">
												<div
											...
											<span className="text-xs text-gray-500">{monthlyBreakdowns.ytd.totals.budgetUsed}% used</span>
										</div>
									</td>
								</tr>
							</tbody>
						...
						<div className="py-12 text-center text-gray-500">No transactions for this year</div>
					)}
				</div>
			</div>
			...
			<div>
				<h2 className="mb-6 text-xl font-bold text-gray-800">Monthly Spending Summaries for {selectedYear}</h2>
						...
						(monthlyBreakdowns?.months ?? []).map((m) => (
							<div key={m.month} className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
								...
								>
									...
									<svg
										className={`h-5 w-5 text-gray-500 transition-transform ${expandedMonths[m.monthName] ? "rotate-180" : ""}`}
										fill="none"
										stroke="currentColor"
										viewBox="0 0 24 24"
									>
											...
											<table className="w-full">
												...
												<tbody>
													...
													<tr className="border-t border-gray-200 bg-green-50 font-semibold">
														...
														<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(m.totals.total)}</td>
														<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(m.totals.budget)}</td>
														<td className="px-4 py-3 text-sm text-gray-800">{formatCurrency(m.totals.budgetLeft)}</td>
														<td className="px-4 py-3">
															<span className="text-xs text-gray-500">{m.totals.budgetUsed}% used</span>
														</td>
													</tr>
												</tbody>
											</table>
											...
											<table className="w-full">
												...
												<tbody>
													<tr className="border-t border-gray-100 hover:bg-gray-50">
														...
														<td className="px-4 py-3 text-sm text-gray-700">{formatCurrency(m.totals.total)}</td>
														<td className={`px-4 py-3 text-sm font-medium ${(m.totals.budgetLeft ?? 0) < 0 ? "text-red-600" : "text-gray-700"}`}>
