...
export function parseCSVToTransactions(
  ...
  if (config.singleColumn) {
    ...
    if (debitIdx < 0) {
      return { ok: false, error: config.debitHeader
        ? `Debit column "${config.debitHeader}" not found in CSV.`
        : "Debit column not configured. Check card settings." };
    }
    if (creditIdx < 0) {
      return { ok: false, error: config.creditHeader
  ...
  for (const row of dataRows) {
    ...
    if (config.singleColumn && amountIdx >= 0) {
      const { value, isDebit } = parseAmount(row[amountIdx] || "");
      if (isDebit) {
        debit = value;
        amount = -value;
      } else {
